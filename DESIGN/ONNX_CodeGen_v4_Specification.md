...
> **Mục tiêu**: Sinh code inference C++/Python từ ONNX model cho mobile platforms (Android/iOS)  
> **Hai chế độ hoạt động**: ONNX-only mode và ONNX+Python mode  
> **Hai giao diện**: CLI và GUI (PySide6)
...
│    - Không phụ thuộc vào GUI hay CLI                           │
│    - Có thể dùng như library                                   │
│    - Unit testable                                             │
│                                                                 │
│  • CLI (onnx_codegen/cli/)                                     │
│    - Thin wrapper quanh Core                                   │
│    - Dành cho automation, scripting                            │
│                                                                 │
│  • GUI (onnx_codegen/gui/)                                     │
│    - Thin wrapper quanh Core                                   │
│    - Dành cho interactive use                                  │
│    - Chỉ phụ thuộc PySide6                                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│
├── core/                    # Core logic (không GUI dependencies)
│   ├── __init__.py
...
**Lưu ý:** Khi user cung cấp Python code, tool KHÔNG sinh Python code mới, chỉ translate sang C++.

### 2.2. Mode B: ONNX only

```
Input:  model.onnx
Output: C++ code + Python code (để verify)

Độ tin cậy: 70-85% (tùy model)
```
...
**Lưu ý:** Khi không có Python code, tool sinh cả Python (để verify) và C++ (để deploy).

### 2.3. Dependencies & Environment

Tool yêu cầu các dependencies sau để hoạt động đầy đủ:
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  REQUIRED (bắt buộc để verify C++ code):                       │
│  ────────────────────────────────────────                       │
│  Python:                                                        │
│  • Python >= 3.9                                               │
│  • onnx >= 1.14.0                                              │
...
│                                                                 │
│  OPTIONAL (cho mobile targets):                                │
│  ─────────────────────────────                                  │
│  • Android NDK        → generate/verify Android code          │
│  • Android Emulator   → run Android code trên PC              │
│  • Xcode (macOS only) → generate/verify iOS code              │
│                                                                 │
│  PLATFORM NOTES:                                                │
│  • Windows/Linux: sinh + verify C++ và Android được           │
│  • Windows/Linux: sinh iOS được, nhưng KHÔNG verify được      │
│  • macOS: sinh + verify tất cả platforms                      │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
**Startup behavior:**
- Tool check tất cả dependencies khi khởi động
- Thiếu required → Error + hướng dẫn install
- Thiếu optional → Vẫn chạy, disable/warning targets tương ứng

---

## 3. Module 1: ONNX Analyzer

### 3.1. Thông tin trích xuất được (100% chính xác)
...
@dataclass
class TensorInfo:
    """Thông tin về một tensor."""
    name: str
    shape: List[int]
    dtype: str
    is_dynamic: bool = False  # True nếu có dimension là -1 hoặc symbolic


@dataclass 
class ONNXModelInfo:
    """Thông tin trích xuất từ ONNX file."""
    # Chắc chắn đúng
...
class ONNXAnalyzer:
    """Phân tích ONNX file để trích xuất thông tin."""
    ...
    def analyze(self, progress_callback=None) -> ONNXModelInfo:
        """
        Phân tích toàn diện ONNX model.
        
        Args:
            progress_callback: Optional callback(percent, message) để báo tiến độ
        """
...
class Architecture(Enum):
    ...
    SSD = auto()
    EFFICIENTDET = auto()
    END_TO_END = auto()  # Model đã có NMS
...
class DetectionResult:
    """Kết quả nhận diện architecture."""
    architecture: Architecture
    confidence: float  # 0.0 - 1.0
    evidence: List[str]  # Lý do đưa ra kết luận
    suggestions: Dict[str, Any]  # Gợi ý config


class ArchitectureDetector:
    """Nhận diện architecture từ ONNX model."""
    
    # Patterns cho từng architecture
    PATTERNS = {
        Architecture.YOLOV8: {
            "output_patterns": [
    ...
    def detect(self) -> DetectionResult:
        ...
        # Chọn architecture có score cao nhất
    ...
    def _create_suggestions(self, arch: Architecture, output_shapes: List) -> Dict:
        """Tạo gợi ý config dựa trên architecture."""
        suggestions = {
            "preprocessing": {
                "color_format": "RGB",
                "normalize": True,
...
Parser để trích xuất preprocessing/postprocessing config từ Python inference code.
...
class ParsedPreprocessing:
    """Thông tin preprocessing trích xuất từ Python code."""
    ...
    # Evidence (dòng code tìm thấy)
    evidence: Dict[str, str] = field(default_factory=dict)


@dataclass
class ParsedPostprocessing:
    """Thông tin postprocessing trích xuất từ Python code."""
...
class PythonCodeParser:
    """
    Parser để trích xuất preprocessing/postprocessing từ Python inference code.
    
    Hỗ trợ các patterns phổ biến:
    ...
    # Patterns để detect preprocessing
    PREPROCESS_PATTERNS = {
        # Resize patterns
    ...
    # Patterns để detect postprocessing
    ...
    def parse(self) -> ParseResult:
        """Parse Python code và trích xuất config."""
        preprocessing = ParsedPreprocessing()
        postprocessing = ParsedPostprocessing()
        errors = []
        warnings = []
    ...
    def _extract_preprocessing(self) -> ParsedPreprocessing:
        """Trích xuất preprocessing config."""
    ...
    def _extract_postprocessing(self) -> ParsedPostprocessing:
        """Trích xuất postprocessing config."""
    ...
    def _extract_model_path(self) -> Optional[str]:
        """Trích xuất đường dẫn model từ code."""
        patterns = [
            re.compile(r'["\']([^"\']+\.onnx)["\']'),
    ...
    def _validate_and_fill(self, prep: ParsedPreprocessing, post: ParsedPostprocessing, 
                          warnings: List[str]):
        """Validate và fill default values cho missing fields."""
...
class UltralyticsParser(PythonCodeParser):
    ...
    Biết trước các patterns của ultralytics package.
    """
...
Config Builder tổng hợp thông tin từ ONNX Analyzer và Python Parser để tạo ModelConfig.
...
class ConfigBuilder:
    """
    Builder để tạo ModelConfig từ nhiều nguồn.
    
    Priority order:
    1. Python code (nếu có) - highest confidence
    ...
    def build(self) -> BuildResult:
        """Build ModelConfig từ available sources."""
    ...
    def _build_from_python(self) -> BuildResult:
        """Build config từ parsed Python code."""
    ...
    def _build_from_heuristic(self) -> BuildResult:
        """Build config từ ONNX heuristics."""
...
### 5.1. Các nguyên tắc thiết kế cốt lõi
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  App này tuân thủ các nguyên tắc thiết kế sau đây để đảm bảo   │
...
```
┌─────────────────────────────────────────────────────────────────┐
│  NGUYÊN TẮC: Chỉ hiện những gì cần thiết tại thời điểm đó      │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ❌ SAI: Hiện tất cả 50 options cùng lúc                       │
│     → User overwhelmed, không biết bắt đầu từ đâu              │
│                                                                 │
│  ✅ ĐÚNG: Hiện từng bước, mở rộng khi cần                      │
│     → User tập trung vào việc đang làm                         │
│                                                                 │
│  ÁP DỤNG:                                                       │
│  • Required inputs (ONNX file) hiện trước, rõ ràng             │
│  • Optional inputs (Python code, Labels) ẩn trong collapsible  │
│  • Advanced settings ẩn, chỉ hiện khi user cần                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  NGUYÊN TẮC: Tách C++ Core và Mobile Wrapper                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
...
│  Step 3: VERIFY     → Test trên PC, đảm bảo logic đúng        │
│  Step 4: GENERATE   → Sinh C++ core code                       │
│                                                                 │
│  → User có thể DỪNG ở đây nếu chỉ cần PC                      │
│                                                                 │
│  PHASE 2: Mobile Wrapper (Optional)                            │
│  ──────────────────────────────────                             │
│  Step 5: MOBILE CONFIG  → Chọn platform, use case              │
│  Step 6: GENERATE       → Sinh JNI/Swift wrapper + app code    │
│                                                                 │
│  LÝ DO TÁCH 2 PHASE:                                           │
│  • C++ core là NỀN TẢNG cho cả PC và Mobile                   │
│  • Verify C++ trên PC trước → đảm bảo logic đúng              │
│  • Mobile code chỉ là WRAPPER, không cần verify lại logic     │
│  • Rõ ràng: C++ = logic, Mobile = integration                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 5.1.3. Sensible Defaults (Mặc định hợp lý)

```
┌─────────────────────────────────────────────────────────────────┐
│  NGUYÊN TẮC: App phải hoạt động tốt với config mặc định       │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  QUICK PATH (80% users):                                       │
│  User chỉ cần: Chọn ONNX file → Click Generate                 │
│  App tự động: Detect architecture → Set defaults → Sinh code   │
│                                                                 │
│  ADVANCED PATH (20% users):                                    │
│  User có thể: Override bất kỳ setting nào                      │
│  App hỗ trợ: Verify trước khi generate                         │
│                                                                 │
│  ÁP DỤNG:                                                       │
...
│  • "Skip to Generate" button cho power users                   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 5.1.4. Immediate Feedback (Phản hồi ngay lập tức)

```
┌─────────────────────────────────────────────────────────────────┐
│  NGUYÊN TẮC: User phải thấy ngay kết quả của action           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ÁP DỤNG:                                                       │
│  • Chọn ONNX → Ngay lập tức hiện model info                   │
│    (architecture, input shape, output shape, file size)        │
│                                                                 │
│  • Đổi config → Ngay lập tức validate                          │
│    (warning nếu giá trị không hợp lệ)                          │
│                                                                 │
│  • Chạy verify → Hiện progress bar + timing + results          │
│    (user không phải đoán app đang làm gì)                      │
│                                                                 │
│  • Generate code → Preview ngay trong app                      │
│    (không cần mở folder để xem)                                │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 5.1.5. Reversible Actions (Có thể hoàn tác)

```
┌─────────────────────────────────────────────────────────────────┐
│  NGUYÊN TẮC: User không sợ thử vì luôn có thể quay lại        │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ÁP DỤNG:                                                       │
│  • Back button để quay lại bước trước                         │
│  • Reset to defaults cho từng section                          │
│  • Verify "Sai" → quay lại Configure, giữ nguyên values       │
│  • Config history để restore (nice to have)                    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 5.1.6. Error Prevention (Ngăn ngừa lỗi)

```
┌─────────────────────────────────────────────────────────────────┐
│  NGUYÊN TẮC: Ngăn lỗi tốt hơn là sửa lỗi                      │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ÁP DỤNG:                                                       │
│  • Disable "Next" button nếu chưa đủ required input           │
│  • Validate input ngay khi user nhập                           │
│  • Warning dialog nếu config có vẻ không hợp lệ               │
│  • Confirm dialog trước khi overwrite existing files          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  "Tôi có ONNX file, muốn sinh code C++ cho PC"                │
│                                                                 │
│  1. Chọn ONNX file                                             │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  "Tôi muốn code chạy đúng trên PC, rồi sinh code cho mobile"  │
│                                                                 │
│  PHASE 1: C++ Core                                             │
│  1. Chọn ONNX file                                             │
│  2. [Optional] Thêm Python inference code, Labels file         │
│  3. Điều chỉnh config nếu cần                                  │
│  4. Import test image → Run verify → Xem kết quả              │
│  5. Kết quả sai? → Điều chỉnh config → Verify lại             │
│  6. Kết quả đúng? → Generate C++ code                          │
│                                                                 │
│  PHASE 2: Mobile (Optional)                                    │
│  7. Click "Generate Android" hoặc "Generate iOS"               │
│  8. Chọn use case (verify 1 ảnh / thư mục / camera)           │
...
┌─────────────────────────────────────────────────────────────────┐
│  USER FLOW C: Mobile Only (cần C++ trước)                      │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  "Tôi chỉ cần code cho Android/iOS"                           │
│                                                                 │
│  → Vẫn phải qua Phase 1 để sinh C++ core                      │
│  → C++ core là nền tảng, mobile code chỉ là wrapper           │
│  → Verify C++ trên PC trước để đảm bảo logic đúng             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│  │     • Bắt buộc phải có                                     ││
│  │     • Dùng để: detect architecture, extract I/O shapes     ││
│  │                                                             ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  ┌─ OPTIONAL ─────────────────────────────────────────────────┐│
...
│  │     • Nếu không: Dùng heuristics + user config             ││
│  │                                                             ││
│  │  📄 Class Labels File (.txt)                               ││
│  │     • Không bắt buộc                                       ││
│  │     • Nếu có: Sinh code với class names                    ││
│  │     • Nếu không: Dùng class indices (0, 1, 2, ...)        ││
│  │                                                             ││
│  │  🖼️ Test Image (.jpg, .png)                                ││
│  │     • Không bắt buộc                                       ││
│  │     • Chỉ cần cho Verify step                              ││
│  │                                                             ││
│  └─────────────────────────────────────────────────────────────┘│
│                                                                 │
│  APP BEHAVIOR:                                                  │
│  • Không có optional inputs → Vẫn sinh code được              │
│  • Có Python code → Sinh code chính xác hơn                   │
│  • Có Labels file → Code có class names                       │
│  • Có Test image → Có thể verify trước khi generate           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│  ║  │                                                                 │  ║ │
│  ║  │   ℹ️ Mobile code (Phase 2) luôn dùng Raw buffer mode.          │  ║ │
│  ║  │      Chọn library ở đây chỉ ảnh hưởng code PC/Desktop.        │  ║ │
│  ║  │                                                                 │  ║ │
│  ║  └─────────────────────────────────────────────────────────────────┘  ║ │
│  ║                                                                       ║ │
...
│        vì camera APIs đã cung cấp decoded pixels.              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### Step 3: Verify (Khuyến khích)
...
│  ║                                                                       ║ │
│  ║  ┌─ Why Verify? ──────────────────────────────────────────────────┐  ║ │
│  ║  │  Test code trên PC TRƯỚC KHI sinh code cho mobile.             │  ║ │
│  ║  │  Đảm bảo logic preprocessing/postprocessing đúng.              │  ║ │
│  ║  └────────────────────────────────────────────────────────────────┘  ║ │
│  ║                                                                       ║ │
│  ║  ┌─ Verification Options ─────────────────────────────────────────┐  ║ │
│  ║  │                                                                 │  ║ │
│  ║  │  [✓] Verify Python code (user-provided)                         │  ║ │
│  ║  │      └─ Chạy detect.py với test image để xác nhận code đúng    │  ║ │
│  ║  │                                                                 │  ║ │
│  ║  │  [✓] Verify C++ code (tool-generated)                           │  ║ │
│  ║  │      └─ Compile + run C++ code, so sánh với Python             │  ║ │
│  ║  │                                                                 │  ║ │
│  ║  │  ℹ️ Nếu có Python code: verify Python trước để đảm bảo         │  ║ │
│  ║  │     logic inference đúng, sau đó verify C++ để đảm bảo         │  ║ │
│  ║  │     code sinh ra khớp với Python.                              │  ║ │
│  ║  │                                                                 │  ║ │
│  ║  └─────────────────────────────────────────────────────────────────┘  ║ │
│  ║                                                                       ║ │
│  ║  ┌─ Test ────────────────────────────────────────────────────────┐   ║ │
...
│  ║  │  └─────────────────────────────┘ └──────────────────────────┘ │   ║ │
│  ║  │                                                                │   ║ │
│  ║  │  ✅ MATCH: Python và C++ results khớp nhau!                   │   ║ │
│  ║  │                                                                │   ║ │
│  ║  └────────────────────────────────────────────────────────────────┘   ║ │
│  ║                                                                       ║ │
│  ║  ┌─ Verification ─────────────────────────────────────────────────┐  ║ │
│  ║  │  Boxes và labels có đúng không?                                │  ║ │
│  ║  │                                                                │  ║ │
│  ║  │  [✗ Sai - Quay lại Configure]        [✓ Đúng - Generate Code] │  ║ │
│  ║  └────────────────────────────────────────────────────────────────┘  ║ │
│  ║                                                                       ║ │
│  ╚═══════════════════════════════════════════════════════════════════════╝ │
│                                                                             │
...
│  ─────────────────────────────────────                          │
│  Step A: Verify Python code (optional nhưng khuyến khích)      │
│    • Chạy user's Python code với test image                    │
│    • User xác nhận kết quả đúng                                │
│    • Mục đích: Đảm bảo code gốc đúng TRƯỚC KHI sinh C++       │
│                                                                 │
│  Step B: Verify C++ code (tool-generated)                      │
│    • Compile + run C++ code sinh ra                            │
│    • So sánh output với Python                                  │
│    • Mục đích: Đảm bảo C++ khớp với Python                     │
│                                                                 │
│  Flow: Python ✓ → C++ ✓ → Results match ✓ → Generate          │
│                                                                 │
│  CASE 2: User KHÔNG cung cấp Python code                       │
│  ────────────────────────────────────────                       │
│  • Tool tự sinh Python test code (từ config)                   │
│  • Chỉ verify C++ vs tool's Python                             │
│  • User phải manual verify kết quả có đúng không               │
│                                                                 │
│  Flow: Tool's Python → C++ ✓ → User confirms → Generate       │
│                                                                 │
│  WHY VERIFY PYTHON CODE?                                        │
│  ───────────────────────                                        │
│  • User's Python code có thể có bug                            │
│  • Tool extract config từ Python → config có thể sai          │
│  • Verify Python trước = đảm bảo baseline đúng                │
│  • Nếu Python sai → C++ cũng sẽ sai (garbage in, garbage out) │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│  ║  ┌─ Phase 1 Complete! ────────────────────────────────────────────┐  ║ │
│  ║  │                                                                │  ║ │
│  ║  │  ✅ C++ code đã được verify và sinh thành công!               │  ║ │
│  ║  │                                                                │  ║ │
│  ║  │  [📁 Open Output Folder]                                       │  ║ │
│  ║  │                                                                │  ║ │
│  ║  │  ───────────────────────────────────────────────────────────  │  ║ │
...
│  ║  └────────────────────────────────────────────────────────────────┘  ║ │
│  ║                                                                       ║ │
│  ║  ┌─ Android: Chọn mục đích sử dụng ───────────────────────────────┐  ║ │
│  ║  │                                                                │  ║ │
│  ║  │   ○ Verify 1 ảnh                                               │  ║ │
│  ║  │     Test nhanh trên device                                     │  ║ │
│  ║  │     Output: ảnh + bboxes, result.txt (YOLO format)             │  ║ │
│  ║  │                                                                │  ║ │
│  ║  │   ○ Verify thư mục ảnh                                         │  ║ │
│  ║  │     Batch evaluation, tính mAP                                 │  ║ │
│  ║  │     Output: ảnh + bboxes, results/*.txt (YOLO format)          │  ║ │
│  ║  │                                                                │  ║ │
│  ║  │   ● Tích hợp camera (production)                               │  ║ │
│  ║  │     Real-time detection từ camera                              │  ║ │
│  ║  │     Output: Không - chỉ return detections                      │  ║ │
│  ║  │                                                                │  ║ │
│  ║  └────────────────────────────────────────────────────────────────┘  ║ │
│  ║                                                                       ║ │
│  ╚═══════════════════════════════════════════════════════════════════════╝ │
...
**iOS tương tự với Swift options.**
...
#### 5.5.1. Nguyên tắc: Tool verify cái tool tạo ra
...
│                                                                 │
│  NGUYÊN TẮC:                                                    │
│  • Tool phải chịu trách nhiệm verify code mà tool sinh ra      │
│  • Không để user phát hiện lỗi lúc deploy                      │
│  • Verify cả Python logic VÀ C++ code khi có thể              │
│                                                                 │
│  VERIFICATION LEVELS:                                           │
│                                                                 │
│  1. Python Logic (LUÔN chạy)                                   │
│     • Verify preprocessing, inference, postprocessing          │
│     • Nhanh: < 1 giây                                          │
│     • Baseline để so sánh với C++                              │
│                                                                 │
│  2. C++ Code (nếu có environment)                              │
│     • Compile code đã sinh                                     │
│     • Chạy với cùng test image                                 │
│     • So sánh kết quả với Python                               │
│     • Đảm bảo C++ hoạt động đúng                               │
│                                                                 │
│  NẾU KHÔNG VERIFY ĐƯỢC C++:                                    │
│  • Warning rõ ràng trong UI                                    │
│  • Vẫn cho generate (user's choice)                           │
│  • Sinh kèm test files để user verify sau                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│               │ ✅ Run   │ ✅ Run*    │ ✅ Run**               │
│                                                                 │
│  * Android: cần Android Emulator (x86_64) hoặc device          │
│  ** iOS: cần iOS Simulator hoặc device                         │
│                                                                 │
│  KẾT LUẬN:                                                      │
│  • iOS code CHỈ có thể verify trên macOS                       │
│  • Trên Windows/Linux: iOS dựa vào C++ core đã verify         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│  ────────────────────────────────────────                       │
│  ✅ Python inference      → Baseline, LUÔN chạy               │
│  ✅ C++ compile + run     → Nếu có CMake + compiler + libs    │
│  ✅ So sánh kết quả       → Python vs C++ phải khớp           │
│                                                                 │
│  → C++ Core PHẢI verify thành công trước khi sinh mobile      │
│                                                                 │
│  PHASE 2: Mobile Wrapper (Step 5-6)                            │
│  ──────────────────────────────────                             │
│  Mobile code = C++ core + platform wrapper                     │
│  • C++ core đã verify ở Phase 1 → logic đúng                  │
│  • Wrapper chỉ là bridge code → ít khả năng lỗi logic        │
│                                                                 │
│  Android:                                                       │
│  • Trên mọi OS: có thể compile với NDK                        │
│  • Trên mọi OS: có thể run với Emulator (nếu có)             │
│                                                                 │
│  iOS:                                                           │
│  • Trên macOS: compile + run với Xcode/Simulator              │
│  • Trên Windows/Linux: CHỈ sinh code, KHÔNG verify được      │
...
#### 5.5.4. iOS Warning UI (trên non-macOS)
...
│  ║  ┌─ ⚠️ Platform Notice ───────────────────────────────────────────┐  ║ │
│  ║  │                                                                │  ║ │
│  ║  │  Bạn đang chạy tool trên Windows/Linux.                       │  ║ │
│  ║  │                                                                │  ║ │
│  ║  │  • iOS code sẽ được sinh nhưng KHÔNG THỂ verify trên máy này │  ║ │
│  ║  │  • C++ core đã được verify ở Phase 1                          │  ║ │
│  ║  │  • iOS wrapper code dựa trên C++ core đã verify              │  ║ │
│  ║  │                                                                │  ║ │
│  ║  │  Để verify iOS code, cần:                                     │  ║ │
│  ║  │  • macOS với Xcode installed                                  │  ║ │
│  ║  │  • iOS Simulator hoặc physical device                         │  ║ │
│  ║  │                                                                │  ║ │
│  ║  └────────────────────────────────────────────────────────────────┘  ║ │
│  ║                                                                       ║ │
│  ║  ┌─ iOS: Chọn mục đích sử dụng ───────────────────────────────────┐  ║ │
│  ║  │   ...                                                          │  ║ │
│  ║  └────────────────────────────────────────────────────────────────┘  ║ │
│  ║                                                                       ║ │
│  ╚═══════════════════════════════════════════════════════════════════════╝ │
...
│                                                                 │
│  IMAGE DISPLAY:                                                 │
│  ✅ Hiện ảnh gốc với bounding boxes overlay                    │
│  ✅ Mỗi class có màu riêng (consistent across app)             │
│  ✅ Label format: "class_name confidence" (e.g., "person 0.94")│
│  ✅ Box border: 2-3px, không fill                              │
│  ✅ Label background: cùng màu với box                         │
│  ✅ Label text: màu trắng                                      │
│                                                                 │
│  DETECTION LIST:                                                │
│  ✅ Table với columns: Class, Confidence, Box                  │
│  ✅ Color indicator khớp với box color                         │
│  ✅ Interactive: hover box → highlight row, và ngược lại       │
│                                                                 │
│  TIMING INFO:                                                   │
│  ✅ Breakdown: Preprocess | Inference | Postprocess            │
│  ✅ Total time                                                  │
│  ✅ Giúp user đánh giá performance                             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  File: image_name.txt (cùng tên với image file)                │
│                                                                 │
│  Format mỗi dòng:                                              │
...
│                                                                 │
│  Trong đó:                                                      │
│  • class_id: index của class (0, 1, 2, ...)                   │
...
```
┌─────────────────────────────────────────────────────────────────┐
│  CHECKLIST: App có tuân thủ các nguyên tắc UX không?           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ✅ Progressive Disclosure                                      │
│     □ Required inputs hiện trước, rõ ràng                      │
│     □ Optional inputs trong collapsible section                │
│     □ Advanced settings ẩn mặc định                            │
│                                                                 │
│  ✅ Two-Phase Wizard                                            │
│     □ Phase 1: Input → Configure → Verify → Generate C++       │
│     □ Phase 2 (optional): Mobile Config → Generate Mobile      │
│     □ Step indicator luôn hiện                                 │
│     □ User biết đang ở bước nào                                │
│                                                                 │
│  ✅ Sensible Defaults                                           │
│     □ Auto-detect từ ONNX file                                 │
│     □ User có thể chỉ: chọn file → Generate                    │
│     □ Không bắt buộc config gì thêm                            │
│                                                                 │
│  ✅ Immediate Feedback                                          │
│     □ Load ONNX → hiện model info ngay                         │
│     □ Run verify → hiện results + timing ngay                  │
│     □ Progress bar cho long operations                         │
│                                                                 │
│  ✅ Reversible Actions                                          │
│     □ Back button mỗi step                                     │
│     □ Reset to defaults button                                 │
│     □ "Sai" → quay lại Configure (giữ values)                 │
│                                                                 │
│  ✅ Escape Hatches                                              │
│     □ "Skip to Generate" cho power users                       │
│     □ "Skip" verify nếu không cần                              │
│                                                                 │
│  ✅ Error Prevention                                            │
│     □ Disable Next nếu chưa đủ input                          │
│     □ Validate input ngay khi nhập                             │
│     □ Warning nếu config có vẻ sai                             │
│     □ Confirm trước khi overwrite files                        │
│                                                                 │
│  ✅ Platform Awareness                                          │
│     □ Warning khi sinh iOS trên non-macOS                     │
│     □ Hiện rõ verification capability theo platform           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
Trên Linux, Qt GUI apps có thể gặp lỗi XCB khi thiếu dependencies hoặc chạy headless:
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Qt hỗ trợ nhiều "platform plugins" để render UI:              │
│                                                                 │
│  ┌─────────────┬────────────────────────────────────────────┐  │
│  │ Plugin      │ Mô tả                                      │  │
│  ├─────────────┼────────────────────────────────────────────┤  │
│  │ xcb         │ Linux X11 - hiện UI trên màn hình         │  │
│  │ wayland     │ Linux Wayland - hiện UI trên màn hình     │  │
│  │ windows     │ Windows - hiện UI trên màn hình           │  │
│  │ cocoa       │ macOS - hiện UI trên màn hình             │  │
│  │ offscreen   │ Render vào memory, KHÔNG cần màn hình     │  │
│  └─────────────┴────────────────────────────────────────────┘  │
│                                                                 │
│  OFFSCREEN MODE:                                                │
│  • App vẫn chạy bình thường (logic, events, widgets, etc.)    │
│  • UI được render vào buffer trong RAM                         │
│  • Không hiện cửa sổ trên màn hình                             │
│  • Dùng cho: headless servers, Docker, CI/CD, testing         │
│                                                                 │
│  VÍ DỤ:                                                         │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │  Server (không có monitor)                               │  │
│  │  ┌─────────┐     ┌─────────┐                            │  │
...
```bash
# Ubuntu/Debian - cài đặt XCB dependencies
sudo apt install \
    libxcb1 \
    libxcb-xinerama0 \
    libxcb-cursor0 \
...
# Debug để xem thiếu library nào
...
```python
# main.py - Entry point với Qt platform handling
...
def setup_qt_platform():
    ...
    if sys.platform != "linux":
        return  # Windows/macOS không cần xử lý
...
```bash
# Normal desktop (có display)
python -m onnx_codegen

# Headless server / Docker (tự động fallback offscreen)
python -m onnx_codegen
...
class DetectionImageViewer(QWidget):
    ...
    def set_detections(self, detections: List[Detection]):
        """Set detections để vẽ lên ảnh."""
        self._detections = detections
        self.update()
    
    def clear(self):
        """Clear image và detections."""
        self._image = None
        self._pixmap = None
        self._detections = []
        self.update()
    ...
    def paintEvent(self, event):
        ...
        # Calculate scale để fit image trong widget
...
class VerificationWidget(QWidget):
    ...
    verification_passed = Signal(object)  # Emit config khi user xác nhận đúng
    verification_failed = Signal()         # Emit khi user nói sai
    back_requested = Signal()              # Emit khi user muốn quay lại
    ...
    def _setup_ui(self):
        ...
        verify_layout.addWidget(QLabel("Boxes và labels có đúng không?"))
        verify_layout.addStretch()
        
        self.wrong_btn = QPushButton("✗ Sai - Quay lại Configure")
        ...
        self.correct_btn = QPushButton("✓ Đúng - Generate Code")
...
@dataclass
class GeneratedFile:
    """Thông tin về một file được sinh."""
    path: str
    content: str
    description: str
...
class CodeGenerator:
    """
    Generator để sinh C++ code từ ModelConfig.
...
Module để verify Python code (user-provided) và C++ code (tool-generated).
...
class PythonVerifier:
    ...
    def verify(self, test_image_path: str, timeout: int = 60) -> VerificationResult:
        """Chạy Python code với test image."""
    ...
    def _parse_detections(self, stdout: str) -> List[Detection]:
        """Parse detections từ stdout. Format: class_name conf [x1, y1, x2, y2]"""
        detections = []
...
class CppVerifier:
    """Verify C++ generated code. Compile và run."""
...
class VerificationOrchestrator:
    """Orchestrator để chạy full verification flow."""
    ...
    def run_full_verification(self, test_image_path: str, labels_path: Optional[str] = None,
        ...
        """Chạy full verification: Python → C++ → Compare."""
        ...
        # Step 1: Verify Python (nếu có và được yêu cầu)
...
Phần này chứa code C++ cho PC/Desktop. Tool sinh code verify cho 1 ảnh, user tự mở rộng nếu cần.

### 7.1. Tổng quan Use Cases và Output
...
│  ─────────┼───────────────────────┼───────────────────────────  │
│  PC       │ Verify 1 ảnh          │ image + YOLO txt            │
│  Android  │ Verify 1 ảnh          │ image + YOLO txt            │
│  Android  │ Verify thư mục        │ images + YOLO txts          │
│  Android  │ Camera integration    │ detections only             │
│  iOS      │ Verify 1 ảnh          │ image + YOLO txt            │
│  iOS      │ Verify thư mục        │ images + YOLO txts          │
│  iOS      │ Camera integration    │ detections only             │
│                                                                 │
│  PC:      Tool sinh code verify 1 ảnh.                         │
│           User tự mở rộng cho batch/video nếu cần.             │
│                                                                 │
│  Mobile:  User chọn 1 trong 3 options.                         │
│           Tool sinh code tương ứng.                            │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
│  └── README.md                 # Hướng dẫn build & mở rộng     │
│                                                                 │
│  USAGE:                                                         │
│  $ mkdir build && cd build                                     │
│  $ cmake .. && make                                            │
...
│  • test_result.jpg: ảnh với bounding boxes                    │
...
```cpp
// verify_single.cpp
// Verify detection trên 1 ảnh, output YOLO format
...
**File này được sinh ra cùng code, hướng dẫn user build và mở rộng:**
...
Phần này chứa code C++ và Kotlin dành riêng cho Android platform.

### 8.1. Android Use Cases và Output Files
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  OPTION 1: Verify 1 ảnh                                        │
│  ──────────────────────                                         │
│  Mục đích: Test nhanh trên device                              │
│  Output: ảnh + bboxes, result.txt (YOLO format)                │
│                                                                 │
│  📁 output/                                                     │
│  ├── cpp/                                                       │
...
│  │   └── SingleImageVerifier.kt   # Verify 1 ảnh, save result │
│  └── README.md                                                  │
│                                                                 │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
│  OPTION 2: Verify thư mục ảnh                                  │
│  ────────────────────────────                                   │
│  Mục đích: Batch evaluation, tính mAP                          │
│  Output: ảnh + bboxes, results/*.txt (YOLO format)             │
│                                                                 │
│  📁 output/                                                     │
│  ├── cpp/                                                       │
...
│  ─────────────────────────────────────────                      │
│  Mục đích: Real-time detection từ camera                       │
│  Output: Không - chỉ return detections để app xử lý           │
│                                                                 │
│  📁 output/                                                     │
│  ├── cpp/                                                       │
│  │   ├── detector.hpp                                          │
...
// Use case: Test nhanh 1 ảnh trên device
...
```kotlin
// BatchVerifier.kt
// Use case: Batch evaluation, tính mAP
...
/**
 * Verify detection trên thư mục ảnh, output YOLO format cho mỗi ảnh.
 * 
 * Usage:
...
/**
 * Simple overlay view để vẽ detection results.
 * Extend View và override onDraw().
 */
...
Phần này chứa code C++/Objective-C++ và Swift dành riêng cho iOS platform.

### 9.1. iOS Use Cases và Output Files
...
│  │   └── SingleImageVerifier.swift # Verify 1 ảnh             │
│  └── README.md                                                  │
│                                                                 │
│  ─────────────────────────────────────────────────────────────  │
│                                                                 │
...
/// Simple overlay view để vẽ detection results trên camera preview.
...
Quay lại phần Core code, đây là các optimizations chung cho tất cả platforms.
...
### 10.2. Vấn đề hiệu năng trên Mobile
...
│  • Thời gian: 100-200ms trên mobile CPU                        │
│                                                                 │
│  Vấn đề chính:                                                 │
│  • Cache miss liên tục (truy cập không liên tục)               │
│  • Không tận dụng SIMD (NEON trên ARM)                         │
│  • Malloc/free mỗi frame                                       │
│  • Division thay vì multiplication                             │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### 10.3. So sánh hiệu năng các phương pháp

| Method | Time (640×640) | Speedup | Ghi chú |
|--------|----------------|---------|---------|
| Naive loop | ~150ms | 1× | Baseline |
| Row-wise copy | ~80ms | 2× | Tốt hơn cache |
| NEON basic | ~25ms | 6× | SIMD 4-8 pixels |
| NEON + vld3 | ~10ms | 15× | Deinterleave hardware |
| vImage/Accelerate | ~5-8ms | 20× | iOS optimized |
| GPU preprocessing | ~2-3ms | 50× | Best, nhưng phức tạp |
 ...
 * - Buffer reuse (không malloc mỗi frame)
 * - SIMD (NEON trên ARM)
...
public:
    ...
    /**
     * Preprocess image với tối ưu tối đa.
     * 
     ...
     * 
     * NOTE: Pointer trả về valid cho đến lần gọi preprocess() tiếp theo
     */
    ...
    // Getters cho letterbox info (để convert coordinates sau)
...
private:
    ...
    // Pre-allocated buffers (QUAN TRỌNG: không malloc mỗi frame)
...
const float* OptimizedPreprocessor::preprocess(
    ...
    // ─────────────────────────────────────────────────────────────
    // Step 3: Reset padding areas (chỉ khi cần)
    // ─────────────────────────────────────────────────────────────
    // Optimization: chỉ reset nếu letterbox thay đổi
    // Trong thực tế, nếu input size cố định, có thể bỏ qua bước này
...
void OptimizedPreprocessor::hwc_to_chw_normalized(
    ...
    const float scale = 1.0f / 255.0f;  // Multiplication thay vì division
...
void OptimizedPreprocessor::hwc_to_chw_neon(
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (; x + 8 <= width; x += 8) {
            ...
            // vld3_u8: Load 24 bytes và deinterleave thành 3 vectors của 8 bytes
            // Đây là key optimization - hardware tự động deinterleave!
            uint8x8x3_t rgb = vld3_u8(src_row + x * 3);
            
            // Chọn channel dựa trên BGR/RGB
...
void OptimizedPreprocessor::resize_vimage(
    ...
    if (channels == 4) {
        ...
        // RGB - vImage không có native RGB, phải convert hoặc dùng planar
...
ImageNetNormalizer::ImageNetNormalizer(
    ...
    // Pre-compute để tránh tính toán mỗi pixel
...
│                                                                 │
│  MEMORY MANAGEMENT                                              │
│  ✅ Pre-allocate buffers (không malloc mỗi frame)              │
│  ✅ Reuse tensors giữa các frames                              │
│  ✅ 16-byte alignment cho SIMD                                 │
│                                                                 │
│  SIMD (ARM NEON)                                               │
│  ✅ Dùng vld3_u8 để deinterleave RGB                           │
│  ✅ Process 8-16 pixels mỗi iteration                          │
│  ✅ Dùng vmlaq_f32 cho fused multiply-add                      │
│                                                                 │
│  MATH OPERATIONS                                                │
│  ✅ Multiplication thay vì division (x * 0.00392 vs x / 255)   │
│  ✅ Pre-compute scale và offset                                │
│  ✅ Tránh branching trong inner loop                           │
│                                                                 │
│  MEMORY ACCESS                                                  │
│  ✅ Process theo row (cache-friendly)                          │
│  ✅ Prefetch nếu cần (__builtin_prefetch)                      │
│  ✅ Avoid random access patterns                                │
│                                                                 │
│  PLATFORM-SPECIFIC                                              │
│  ✅ iOS: vImage, Accelerate, Metal                             │
│  ✅ Android: RenderScript, Vulkan Compute                      │
│  ✅ Consider GPU preprocessing nếu có sẵn                      │
│                                                                 │
│  RESIZE OPTIMIZATION                                            │
│  ✅ Nearest neighbor cho real-time (fastest)                   │
│  ✅ Bilinear cho quality/speed balance                         │
│  ✅ GPU resize khi có thể                                      │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
# Predefined errors với suggestions
ERROR_TEMPLATES = {
...
class ErrorHandler:
    """Handler để display errors trong GUI."""
    
    @staticmethod
    def format_for_gui(error: ONNXCodeGenError) -> str:
        """Format error cho hiển thị trong GUI."""
...
│  ──────────────────────────────────                             │
│  Step 5: Mobile Config  → Chọn platform, use case              │
│  Step 6: Generate       → Sinh wrapper code                    │
│                                                                 │
│  Output (thêm vào):                                            │
...
│  • verify_single.cpp     - Test với 1 ảnh                      │
│  • CMakeLists.txt        - Build config                        │
│  • README.md             - Hướng dẫn build & extend           │
│                                                                 │
│  PHASE 2: Mobile Wrapper (optional)                            │
│  ──────────────────────────────────                             │
│                                                                 │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  REQUIRED (tool không chạy/verify nếu thiếu):                  │
│  • Python >= 3.9 + packages (onnx, onnxruntime, PySide6, etc.) │
│  • CMake + C++ compiler                                        │
│  • ONNX Runtime C++ libs                                       │
│  • OpenCV C++ libs       → verify C++ code trên PC            │
│                                                                 │
│  OPTIONAL (cho mobile targets):                                │
...
│                                                                 │
│  PLATFORM NOTES:                                                │
│  • Windows/Linux: Có thể sinh + verify C++ và Android         │
│  • Windows/Linux: Có thể sinh iOS nhưng KHÔNG verify được     │
│  • macOS: Có thể sinh + verify tất cả platforms               │
│                                                                 │
...
│  ─────────────────                                              │
│  Step 1: INPUT                                                 │
│  • Chọn ONNX file (required)                                   │
│  • [Optional] Python code, Labels file                         │
│                                                                 │
│  Step 2: CONFIGURE                                             │
│  • Chọn C++ options (OpenCV / stb_image / Raw buffer)         │
...
│  • Chọn use case (verify 1 ảnh / thư mục / camera)           │
│                                                                 │
│  Step 6: GENERATE MOBILE                                       │
│  • Sinh wrapper code (JNI/Swift)                              │
│  • Sinh app code (Kotlin/Swift)                               │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
> - **Mục tiêu**: Sinh code inference C++ từ ONNX model
> - **Workflow**: 2 phases - C++ Core (required) + Mobile Wrapper (optional)
> - **Nguyên tắc**: Verify C++ trên PC trước, mobile code chỉ là wrapper
